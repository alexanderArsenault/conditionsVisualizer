  ...
  <div>
    ...
    <svg :width="container" :height="container" v-bind:class="{warning: minValue==0}">
      <g :transform="`translate(${(container)/2}, ${container/2})`">
        ...
        <!-- Axial Axis + labels -->
        <g v-for="(tick, i) in axialTicks" :key="'axial-group' + i" :transform="`rotate(${axialRotation(tick)})` ">
          <line 
            :x2="(width / 2) - (width * 0.02)"
            class="axis"
          />
          ...
          <text
            ...
            :transform="`rotate(${+ -1 * (axialRotation(tick))}, ${(width / 2) + 5}, 0) translate(0,4) `"
          >
            {{tick + '%'}}
          </text>
        </g>
        ...
        <!-- coloured spinner -->
          <path v-for="(x, i) in values" v-bind:key="x.k" :d="path(i)" :fill="color(x)" />
...
export default {
  ...
  methods: {
    ...
    radialAxis(i) {
      ...
      return ((this.height * 0.3 + i * this.height * 0.05) + (this.height * 0.3 + (i + 1) * this.height * 0.05 - this.height * 0.01)) / 2
    },

    axialRotation(tickvalue) {
      // As established in angularScale
      let availableRange = 360 * 0.80
